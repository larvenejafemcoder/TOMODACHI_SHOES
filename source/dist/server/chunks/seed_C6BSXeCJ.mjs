...
import { b as batchGenerateMarketData } from './market-sim_CDFBtqHE.mjs';
...
const POST = async ({ request }) => {
  try {
    ...
    const count = Math.min(body.count || 1e3, 5e4);
    ...
    try {
      ...
      imageFiles = files.filter((f) => /\.(jpg|jpeg|png|webp|svg|avif)$/i.test(f)).sort((a, b) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const doc = {
        ...
        isActive: true,
        isFeatured: idx % 10 === 0,
        viewCount: Math.floor((idx * 7919 + 1) % 5e3),
    ...
    return new Response(JSON.stringify({
      ...
      priceRange: {
        min: `¥${89e4.toLocaleString()}`,
        max: `¥${4e6.toLocaleString()}`
      }
    }), {
      status: 200,
