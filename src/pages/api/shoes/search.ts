...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    try {
      ...
      const hits = results.hits.map((hit: Record<string, unknown>) => ({
        ...hit,
        _highlight: results.hits.find((h: Record<string, unknown>) => h.id === hit.id)?._formatted,
      }));
      ...
      // Fallback to MongoDB text search if Meilisearch is down
      ...
      if (sort) {
        ...
        sortObj.score = { $meta: 'textScore' } as unknown as 1 | -1;
      } else {
        sortObj.createdAt = -1;
      }
